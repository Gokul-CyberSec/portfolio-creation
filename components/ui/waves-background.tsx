...
class Noise {
  ...
  seed(seed: number) {
    ...
    for (let i = 0; i < 256; i++) {
      const base = this.p[i] ?? 0
      const v = i & 1 ? base ^ (seed & 255) : base ^ ((seed >> 8) & 255)
      this.perm[i] = this.perm[i + 256] = v
      this.gradP[i] = this.gradP[i + 256] = this.grad3[v % 12] ?? this.grad3[0]
    }
  }
  ...
  perlin2(x: number, y: number) {
    ...
    X &= 255
    Y &= 255
    const grad00 = this.gradP[X + this.perm[Y]!] ?? this.grad3[0]
    const grad01 = this.gradP[X + this.perm[Y + 1]!] ?? this.grad3[0]
    const grad10 = this.gradP[X + 1 + this.perm[Y]!] ?? this.grad3[0]
    const grad11 = this.gradP[X + 1 + this.perm[Y + 1]!] ?? this.grad3[0]
...
export function Waves({
  ...
  const boundingRef = useRef<BoundingBox>({ width: 0, height: 0, left: 0, top: 0 })
  ...
  useEffect(() => {
    ...
    function setSize() {
      ...
      canvasEl.width = boundingRef.current.width
      canvasEl.height = boundingRef.current.height
    }
